...
class OrganizeImportsCommand {
  ...
  Future<void> execute(ArgResults results) async {
    ...
    if (targetFile == null || targetFile.isEmpty) {
      ...
      logger.info('Usage: qftools organize-imports <path/file.dart> [--barrel]');
      return;
    }
  ...
  Future<String?> _selectFileInteractively() async {
    ...
    if (!libDir.existsSync()) {
      logger.error('lib/ directory not found. Make sure you\'re in a Flutter project root.');
      return null;
    }
    ...
    while (true) {
      ...
      if (selection != null && selection >= 1 && selection <= dartFiles.length) {
        return dartFiles[selection - 1];
      } else {
        print('Invalid selection. Please enter a number between 1 and ${dartFiles.length}.');
      }
    }
  }
  ...
  Future<void> _organizeImports(String filePath, {bool useBarrelFiles = false}) async {
    ...
    try {
      ...
      final organizedImports = _organizeImportsByCategory(imports, filePath, useBarrelFiles: useBarrelFiles);
      
      // Combine organized imports with the rest of the file
      final organizedContent = _buildOrganizedContent(organizedImports, nonImportLines);
  ...
  List<String> _organizeImportsByCategory(List<String> imports, String filePath, {bool useBarrelFiles = false}) {
    ...
    for (final import in imports) {
      ...
      } else if (import.contains("import '../") || import.contains("import './")) {
    ...
    if (projectImports.isNotEmpty) {
      organized.addAll(_organizeProjectImports(projectImports, filePath, useBarrelFiles: useBarrelFiles));
    }
  ...
  List<String> _organizeProjectImports(List<String> projectImports, String filePath, {bool useBarrelFiles = false}) {
    final organized = <String>[];
  ...
  String _buildOrganizedContent(List<String> organizedImports, List<String> nonImportLines) {
    final result = <String>[];
